...
That same settings file sets **`env: { ENABLE_TOOL_SEARCH: "false" }`**, which is load-bearing (D-149). Claude Code defers MCP tools by default: the model is handed the tools' *names* in a `deferred_tools_delta` attachment and must call `ToolSearch` to load a definition before it can call anything. Under that default the arms measured tool *discovery* rather than MCP value — Haiku never searched in 24 trials and so never reached a server, and roughly 40% of Sonnet's MCP-arm trials were baseline trials wearing an MCP label. Loading upfront is what "this variant has the MCP available" was always meant to assert; at ten tools we are far from needing the context-window optimisation deferral exists for. The regime is a named part of the variant-version hash, so results from the two can never be pooled, and `EVAL_TOOL_SEARCH=1` restores deferral deliberately for the separate question of whether a model discovers a server nobody pointed it at.
...
- **Budget guardrail of $40 per full run** (D6). Raised from $25 after the P1 calibration run measured `cc-both` at $29.71. Phase 1 then measured a single-eval 4-variant matrix at $35.18 on average, which makes D6 in practice a **per-eval** cap rather than a per-run one. Rather than raise it — the number is doing useful work at that scale — §10.1 tiers the suite so a routine gate run is ~$150 and the expensive tasks are opt-in. **This cap is not enforced anywhere.** Earlier revisions of this line claimed "the harness aborts when exceeded"; nothing in `agent-eval.config.ts`, `lib/`, or the framework implements it, and it was found by looking rather than by it failing to fire (D-150, lesson co). Until it is built, D6 is a budgeting convention that a human applies by choosing what to invoke — a 20-eval sweep is 60 unattended trials and will run to completion whatever it costs.
