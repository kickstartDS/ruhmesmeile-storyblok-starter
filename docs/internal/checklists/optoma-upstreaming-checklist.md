...
- [ ] A.0 Create branch off clean `main`
- [x] A.1 Investigate diffs for each Batch A file (generic vs brand-tainted)
- [ ] A.2 Port `scripts/tokensToCss.mjs` fontFamily quoting fix
- [ ] A.3 Port `scripts/componentTokensToCss.mjs` + `componentTokensToCss.d.mts`
- [ ] A.4 Port `scripts/tokensToCss.d.mts` + `src/scss.d.ts`
- [ ] A.5 Port `scripts/extractComponentTokenCatalog.cjs`
- [ ] A.6 Port `scripts/extractSemanticTokenCatalog.cjs`
- [ ] A.7 Add `package.json` scripts: `component-token-catalog`, `semantic-token-catalog` + build wiring
- [ ] A.8 Add `rollup.config.mjs` copy entries (catalog JSONs + tooling; **not** `token-graph.json`)
- [ ] A.9 Regenerate `component-token-catalog.json` + `semantic-token-catalog.json` in-template (ADR-005)
- [ ] A.10 Build design-system; confirm catalogs generate + build stays green
- [ ] A.11 Add changeset
- [ ] A.12 Commit + open PR
