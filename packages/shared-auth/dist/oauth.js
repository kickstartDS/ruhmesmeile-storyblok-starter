 ...
 */
export function wwwAuthenticateHeader(req) {
    const base = getBaseUrl(req);
    return `Bearer resource_metadata="${base}/.well-known/oauth-protected-resource"`;
}
// ── HTML template for the authorize page ──────────────────────────────────
function renderAuthorizePage(serviceName, params, error) {
    const e = (key) => escapeHtml(params[key] || "");
...
export function createOAuthMiddleware(config = {}) {
    ...
    return async function handleOAuth(req, res) {
        ...
        if (url.pathname === "/.well-known/oauth-authorization-server-metadata" &&
            req.method === "GET") {
            sendJson(res, 200, {
                issuer: base,
        ...
        if (url.pathname === "/register" && req.method === "POST") {
            ...
            clients.set(clientId, {
                clientId,
                redirectUris,
                clientName: registration.client_name || undefined,
            });
            sendJson(res, 201, {
                client_id: clientId,
        ...
        if (url.pathname === "/authorize") {
            if (req.method === "GET") {
                ...
                if (!params.redirect_uri ||
                    ...
                    res.end("Missing required parameters: redirect_uri, client_id, code_challenge, response_type");
                    return true;
                }
                ...
                if (client?.clientName)
                    params.client_name = client.clientName;
            ...
            if (req.method === "POST") {
                ...
                if (!user) {
                    ...
                    if (client?.clientName)
                        params.client_name = client.clientName;
                    res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
                    res.end(renderAuthorizePage(serviceName, params, "Invalid or expired token. Please paste a valid JWT."));
                    return true;
                }
        ...
        if (url.pathname === "/token" && req.method === "POST") {
            ...
            const accessToken = stored.jwt === "__noauth__" ? "__noauth__" : stored.jwt;
