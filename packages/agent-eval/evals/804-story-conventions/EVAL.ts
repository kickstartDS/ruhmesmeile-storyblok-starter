...
var define_FIXTURE_DIGESTS_default = {
  "src/components/price-tag/PriceTagComponent.tsx":
    "3e4812222b31c1c970d48fb588f2268e1b823dc46213db408a88b787e353b38c",
  "src/components/price-tag/_price-tag-tokens.scss":
    "603eb2a013cc6c7dd52a0eedf2596f467ed424b72dc314dfc283c63bb694db0b",
  "src/components/price-tag/price-tag-tokens.json":
    "b6e2fa679467020613601e7e7e48343f0ef22f8582b8f4f83110e932bcfc057b",
  "src/components/price-tag/price-tag.schema.dereffed.json":
    "1d04eceff564958fb0d6162dc7809bbe3698935ed5aeaacca67fd6a0283e8345",
  "src/components/price-tag/price-tag.schema.json":
    "1d04eceff564958fb0d6162dc7809bbe3698935ed5aeaacca67fd6a0283e8345",
  "src/components/price-tag/price-tag.scss":
    "17c7b2023df46ea875a0c2e4790cc0dc46f8f6b8680965e70e70737ebaa6bc66",
...
// lib/eval-harness/sources/804-story-conventions.ts
import {
...
var STORY_FILE = `${harness.dir}/PriceTag.stories.tsx`;
var DOC_FILE = `${harness.dir}/PriceTag.mdx`;
var SHIPPED = {
...
var DOCUMENTED_PROPS = ["amount", "note.text", "period", "variant"];
async function storyModule() {
...
test("the component has a story file", () => {
  expect(existsSync2(STORY_FILE)).toBe(true);
});
test("the story documents this component", async () => {
  const module = await storyModule();
  const specifier = `./${FILES.component}`;
  const componentModule = await import(specifier);
...
test("the controls are derived from the schema rather than restated by hand", async () => {
  const { getArgsShared } = await import("@kickstartds/core/lib/storybook");
...
test("the documentation page is written", async () => {
  expect(existsSync2(DOC_FILE)).toBe(true);
...
test("no unrelated files are added to the component", () => {
  ...
  expect(
    readdirSync2(harness.dir).filter((name) => !allowed.has(name)),
  ).toEqual([]);
});
