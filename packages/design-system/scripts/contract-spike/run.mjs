...
const sha = (p) =>
  existsSync(p) ? "sha256:" + createHash("sha256").update(readFileSync(p)).digest("hex").slice(0, 16) : null;
...
console.log(`viewport:   ${VIEWPORT.width}×${VIEWPORT.height} (desktop-only)\n`);
...
for (const id of components) {
  ...
  console.log(
    `  ${pad(id, 12)} ${lpad(Object.keys(s.api.props).length, 3)} props  ` +
      ...
      (s.unparsedTokens.length ? `  ⚠ ${s.unparsedTokens.length} unparsed` : "")
  );
}
...
for (const [storyId, obs] of Object.entries(observations))
  writeFileSync(join(OUT, ".observations", `${storyId}.json`), JSON.stringify(obs, null, 1));

console.log("\nPass 3 — reconciliation");
const report = { format: "kickstartds/component-contract@1", viewport: VIEWPORT, components: {} };
...
for (const [id, s] of statics) {
  ...
  writeFileSync(join(OUT, id, `${id}.contract.json`), JSON.stringify(contract, null, 2) + "\n");
  writeFileSync(join(OUT, id, `${id}.brief.md`), buildBrief(contract));
  ...
  report.components[id] = {
    ...
    axes: derived.axes.map((a) => ({
      prop: a.prop,
      joined: a.values.filter((v) => v.class !== null || v.tokenSegment !== null).length,
      total: a.values.length,
      issues: a.values.flatMap((v) => (v.issues || []).map((i) => `${a.prop}:${v.api} ${i}`)),
    })),
...
writeFileSync(join(OUT, "report.json"), JSON.stringify(sortKeys(report), null, 2) + "\n");
...
console.log(`\n${"─".repeat(78)}\nANATOMY DERIVATION\n`);
console.log(`  ${pad("component", 12)} ${lpad("parts", 5)} ${lpad("inDflt", 7)} ${lpad("cond", 5)}   naming provenance`);
for (const [id, c] of rows) {
  const naming = Object.entries(c.naming)
    .sort()
    .map(([k, v]) => `${k}:${v}`)
    .join("  ");
  console.log(`  ${pad(id, 12)} ${lpad(c.parts.total, 5)} ${lpad(c.parts.inDefault, 7)} ${lpad(c.parts.conditional, 5)}   ${naming}`);
}

console.log(`\n${"─".repeat(78)}\nVOCABULARY JOIN (api ↔ class ↔ tokenSegment)\n`);
for (const [id, c] of rows) {
  for (const a of c.axes) {
    console.log(`  ${pad(id, 12)} ${pad(a.prop, 14)} ${a.joined}/${a.total} joined${a.issues.length ? "   ⚠ " + a.issues.join("; ") : ""}`);
  }
}

console.log(`\n${"─".repeat(78)}\nBINDING MECHANISMS\n`);
const allMech = [...new Set(rows.flatMap(([, c]) => Object.keys(c.mechanisms)))].sort();
console.log(`  ${pad("component", 12)} ${allMech.map((m) => lpad(m.slice(0, 11), 12)).join("")}`);
for (const [id, c] of rows)
  console.log(`  ${pad(id, 12)} ${allMech.map((m) => lpad(c.mechanisms[m] || "·", 12)).join("")}`);

console.log(`\n${"─".repeat(78)}\nCOVERAGE\n`);console.log(`  ${pad("component", 12)} ${lpad("score", 6)} ${lpad("proven", 7)} ${lpad("possible", 9)}   missing`);
for (const [id, c] of rows) {
...
const failures = rows.flatMap(([id, c]) => c.failedStories.map((f) => `${id}: ${f.id}`));
if (failures.length) {
...
console.log(`  ${pad("component", 12)} ${lpad("contract", 9)} ${lpad("specs yaml", 11)}`);
for (const [id, c] of rows) {
  const specs = join(OUT, id, "reference.specs.api.yaml");
  const size = existsSync(specs) ? readFileSync(specs).length : 0;
  console.log(`  ${pad(id, 12)} ${lpad(c.contractBytes, 9)} ${lpad(size || "—", 11)}`);
}
